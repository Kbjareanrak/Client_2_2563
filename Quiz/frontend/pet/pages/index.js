...
export default () => {
      ...
      if (pets && pets.length)
          return (pets.map((pet, index) =>
              (<li key={index}>
                  ...
                  <button className={styles.button-del} onClick={() => deletePet(pet.id)}> Delete </button>
                  <button className={styles.button-update} onClick={() => getPet(pet.id)}>Get</button>
                  <button className={styles.button-update} onClick={() => updatePet(pet.id)}>Update</button>
              </li>)
          ))
      else {
  ...
  return (
      <div>
          ...
          <button className={styles.button-add} onClick={ () => addPet(typeName, age ,weight, price)}>Add new Pet</button>
      </div>
  )
}
